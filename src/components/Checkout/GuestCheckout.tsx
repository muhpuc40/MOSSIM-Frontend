...
const GuestCheckout = () => {
    ...
    const handlePlaceOrder = async () => {
        ...
        if (!name.trim() || !phone.trim() || !line1.trim() || !city.trim() || !country.trim()) {
            ...
            <div className="cart-block md:py-10 py-10">
                <div className="container">
                    ...
                    <div className="login bg-surface py-3 px-4 flex items-center justify-between rounded-lg mb-8">
                        <span className="caption1 text-secondary">Already have an account?</span>
                        <Link href="/login" className="text-button hover-underline">Login</Link>
                    </div>
                            {/* Contact */}
                            ...
                            <div className="grid sm:grid-cols-2 gap-4 mb-8">
                                ...
                                <input className={`${inputClass} sm:col-span-2`} type="email" placeholder="Email"
                            ...
                            <div className="grid sm:grid-cols-2 gap-4 mb-8">
                                <input className={`${inputClass} sm:col-span-2`} type="text" placeholder="Street Address *"
                                       value={line1} onChange={e => setLine1(e.target.value)} />
                                <input className={`${inputClass} sm:col-span-2`} type="text" placeholder="Apartment, suite, etc. (optional)"
                                       value={line2} onChange={e => setLine2(e.target.value)} />
                                <input className={inputClass} type="text" placeholder="Town/City *"
                                       value={city} onChange={e => setCity(e.target.value)} />
                                <input className={inputClass} type="text" placeholder="State/Division (optional)"
                                       value={state} onChange={e => setState(e.target.value)} />
                                <input className={inputClass} type="text" placeholder="Postal Code (optional)"
                                            ...
                                            <div className="space-y-2">
                                                <label className={`flex items-center justify-between p-2.5 rounded-lg border cursor-pointer duration-200 ${subtotal >= FREE_SHIP_AT ? 'border-black bg-white' : 'border-line opacity-50 cursor-not-allowed'}`}>
                                                    <div className="flex items-center gap-2">
                                                        <input type="radio" name="guest-ship" disabled={subtotal < FREE_SHIP_AT} checked={subtotal >= FREE_SHIP_AT} readOnly />
                                                ...
                                                <label className={`flex items-center justify-between p-2.5 rounded-lg border cursor-pointer duration-200 ${shipType === 'inside' && subtotal < FREE_SHIP_AT ? 'border-black bg-white' : 'border-line'} ${subtotal >= FREE_SHIP_AT ? 'opacity-50' : ''}`}>
                                                    <div className="flex items-center gap-2">
                                                        <input type="radio" name="guest-ship"
