                        ...
                        {cartState.cartArray.length === 0 ? (
                            ...
                            cartState.cartArray.map((product) => (
                                <CartLine
                                    ...
                                    onSwitchVariant={(to) => switchVariant(product.variant_id, to)}
                                />
                            ))
                        )}
                    </div>
...
const CartLine = ({ product, onRemove, onUpdateQty, onSwitchVariant }: CartLineProps) => {
    ...
    const handleDecrease = () => { if (qty > 1) onUpdateQty(qty - 1); };

    return (
        <div className="item py-4 border-b border-line">
            <div className="max-md:hidden flex items-center w-full">
                <div className="bg-img flex-shrink-0 rounded-lg overflow-hidden mr-3" style={{ width: 80, height: 100 }}>
                    <Image src={product.images[0]} width={160} height={200} alt={product.name} className="w-full h-full object-cover" />
                </div>
                <div className="flex-shrink-0 flex flex-col gap-0.5 mr-3" style={{ width: 170 }}>
                    <div className="text-button line-clamp-2 leading-snug">{product.name}</div>
                    <div className="text-title font-semibold">BDT {product.price}</div>
                </div>
                <div className="flex-shrink-0 flex flex-col gap-1 mr-3" style={{ width: 130 }}>
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                        {product.variation?.map((v: any, i: number) => (
                            <button
                                ...
                                className={`w-7 h-7 rounded-full border-2 transition-all duration-200 ${selectedColor === v.color ? "border-black scale-110" : "border-line hover:border-gray-400"}`}
                            />
                        ))}
                    </div>
                </div>
                <div className="flex-shrink-0 flex flex-col gap-1 mr-3" style={{ width: 190 }}>
                ...
                <div className="flex items-center ml-auto flex-shrink-0">
                    <div className="flex flex-col gap-1">
                        ...
                        <div className="quantity-block flex items-center border border-line rounded-lg overflow-hidden">
                            <button onClick={handleDecrease} disabled={qty <= 1} className="w-9 h-9 flex items-center justify-center text-secondary hover:bg-black hover:text-white transition-colors disabled:opacity-30 disabled:cursor-not-allowed">
                                <Icon.Minus size={12} weight="bold" />
                            </button>
                            <span className="body1 font-semibold w-9 h-9 flex items-center justify-center border-x border-line text-center">{qty}</span>
                            <button onClick={handleIncrease} className="w-9 h-9 flex items-center justify-center text-secondary hover:bg-black hover:text-white transition-colors">
                                <Icon.Plus size={12} weight="bold" />
                            </button>
                        </div>
                    </div>
                    <button onClick={onRemove} className="remove-cart-btn caption1 font-semibold text-red underline cursor-pointer ml-3 self-center" style={{ marginTop: "12px" }}>
                        Remove
                    </button>
                </div>
            </div>
            ...
            <div className="md:hidden flex items-start gap-4">
                <div className="bg-img w-[80px] h-[100px] flex-shrink-0 rounded-lg overflow-hidden">
                    <Image src={product.images[0]} width={200} height={260} alt={product.name} className="w-full h-full object-cover" />
                </div>
                <div className="flex-1 min-w-0 flex flex-col gap-2">
                    <div className="flex items-start justify-between gap-2">
                        <div className="text-button line-clamp-2 leading-snug">{product.name}</div>
                        <button onClick={onRemove} className="remove-cart-btn caption1 font-semibold text-red underline cursor-pointer flex-shrink-0">Remove</button>
                    </div>
                    <div className="text-title font-semibold">BDT {product.price}</div>
                    {product.variation && product.variation.length > 0 && (
                        <div>
                            <p className="caption2 text-secondary mb-1">Color: <span className="text-black font-semibold">{selectedColor}</span></p>
                            <div className="flex items-center gap-2 flex-wrap">
                                {product.variation.map((v: any, i: number) => (
                                    <button key={i} title={v.color} onClick={() => handleColorChange(v.color)} style={{ backgroundColor: v.colorCode }} className={`w-7 h-7 rounded-full border-2 transition-all duration-200 ${selectedColor === v.color ? "border-black scale-110" : "border-line hover:border-gray-400"}`} />
                                ))}
                            </div>
                        </div>
                    )}
                    {product.sizes && product.sizes.length > 0 && (
                        <div>
                            <p className="caption2 text-secondary mb-1">Size: <span className="text-black font-semibold">{selectedSize}</span></p>
                            <div className="flex items-center gap-1.5 flex-wrap">
                                {(product.sizes as string[]).map((size, i) => {
                                    const available = availableSizes.includes(size);
                                    return (
                                        <button key={i} disabled={!available} onClick={() => available && handleSizeChange(size)} className={`min-w-[36px] h-8 px-2 flex items-center justify-center caption2 rounded-md border transition-all duration-200 ${!available ? "opacity-30 cursor-not-allowed line-through border-line" : selectedSize === size ? "border-black bg-black text-white" : "border-line hover:border-black cursor-pointer"}`}>
                                            {size}
                                        </button>
                                    );
                                })}
